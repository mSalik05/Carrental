...
<style>
        /* Hide the calendar icon on date inputs in Webkit browsers (Chrome, Safari) */
        input[type="date"]::-webkit-calendar-picker-indicator {
            display: none;
            -webkit-appearance: none;
        }

        /* Hide the calendar icon on date inputs in Firefox */
        input[type="date"]::-moz-calendar-picker-indicator {
            display: none;
        }

        /* Hide the calendar icon on date inputs in Edge */
        input[type="date"]::-ms-clear {
            display: none;
        }

        /* General hide for other browsers */
        input[type="date"] {
            position: relative;
            z-index: 1;
        }
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-datepicker/1.9.0/js/bootstrap-datepicker.min.js"></script>
    ...
    <!-- Initialize the date picker for 'fromdate' and 'todate' fields -->
    <script>
    $(document).ready(function() {
        // Initialize the date picker for 'fromdate'
        $('input[name="fromdate"]').datepicker({
            startDate: new Date(), // Set the start date to today
            autoclose: true,
            todayHighlight: true,
            format: 'yyyy-mm-dd' // Ensures the date format matches your database format
        }).on('changeDate', function(selected) {
            // Set the start date of 'todate' based on the selected 'fromdate'
            var minDate = new Date(selected.date.valueOf());
            $('input[name="todate"]').datepicker('setStartDate', minDate);
        });

        // Initialize the date picker for 'todate'
        $('input[name="todate"]').datepicker({
            startDate: new Date(), // Initially set the start date to today
            autoclose: true,
            todayHighlight: true,
            format: 'yyyy-mm-dd' // Ensures the date format matches your database format
        });
